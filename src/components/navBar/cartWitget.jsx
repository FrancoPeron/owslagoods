...
const CartWidget = ()=>{
    ...
    const [arrayProductsBag] = useState([
        {
            id:"item_1",
            img: product01,
    ...
    const checkout = ()=> {
        console.table(arrayProductsBag)
    };
    ...
    const btn_show = ()=> {
        ...
        setTimeout((e)=>{
            ...
            document.querySelector("#offcanvas").classList.add("offcanvas--active")
            document.querySelector("#offcanvas-background").classList.add("offcanvas-background--active")

        },50)
    }
    const btn_hide = ()=> {

        document.querySelector("#offcanvas-background").classList.remove("offcanvas-background--active")
        document.querySelector("#offcanvas").classList.remove("offcanvas--active")
    ...
    return(
        <>
            <svg className='btn_bag' onClick={btn_show} width="24" height="24" viewBox="0 0 30 30" fill="none" xmlns="http://www.w3.org/2000/svg">
...

            {
                show?
                    <>
                        <div onClick={btn_hide} id='offcanvas-background' className='offcanvas-background'></div>

                        <div id='offcanvas' className="bag">
                            <div className="bag__header">
                                <h2 className="bag__header-title">My Bag</h2>
                                <button onClick={btn_hide} type="button" className="btn-close" offCanvas="hide"></button>
                            </div>
                            <div className="bag__body">
                                {
                                    arrayProductsBag.map( (item) => (
